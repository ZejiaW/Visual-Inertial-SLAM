...
def visualize_slam(ax, pose, landmarks, path_name="Path", show_ori=False):
    '''
    function to visualize the process of SLAM in 2D
    '''
    n_pose = pose.shape[2]
    ax.plot(pose[0, 3, :], pose[1, 3, :], 'b-', label=path_name)
    ax.scatter(pose[0, 3, 0], pose[1, 3, 0], marker='o', label="start point", s=100)
    ax.scatter(pose[0, 3, -1], pose[1, 3, -1], marker='s', label="end point", s=100)
    ax.scatter(landmarks[0,:], landmarks[1,:], label="landmarks", s=60.0, c='m', marker='*')
    ...
    if show_ori:
        ...
        dy = np.sin(yaw_list)
        dx, dy = [dx, dy] / np.sqrt(dx**2 + dy**2)
        ax.quiver(pose[0, 3, select_ori_index], pose[1, 3, select_ori_index], dx, dy, color="r",units="xy",width=1)
